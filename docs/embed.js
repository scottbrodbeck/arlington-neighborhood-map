...
function parsePins() {
  const raw = new URLSearchParams(location.search).get('pins') || '';
  const pins = [];
...
if (pins.length === 1) {
  ...
  view.bounds = bounds;
  view.fitBoundsOptions = { padding: 48, maxZoom: 15 };
} else {
...
for (const pin of pins) {
  ...
  const popup = new maplibregl.Popup({ offset: 30 }).setDOMContent(content);
  const marker = new maplibregl.Marker({ color: '#c33' })
    .setLngLat([pin.lng, pin.lat])
    .setPopup(popup)
    .addTo(map);
  if (pins.length === 1) marker.togglePopup();
